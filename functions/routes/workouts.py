...
def create_workouts_app():
    ...
    def start_workout(user_id):
        try:
            ...
            # BOLT: Pre-pass loop to collect missing exercise references for batch fetching
            missing_name_refs_map = {}
            for exercise in exercises:
                ...
                if not name and exercise_id and exercise_id not in missing_name_refs_map:
                    missing_name_refs_map[exercise_id] = db.collection("users").document(user_id).collection("exercises").document(exercise_id)

            exercise_names_map = {}
            if missing_name_refs_map:
                try:
                    for doc in db.get_all(list(missing_name_refs_map.values())):
                        if doc.exists:
                            exercise_names_map[doc.id] = doc.to_dict().get("name")
                except Exception as e:
                    logging.error(f"Failed to batch fetch exercises: {e}")
