...
## 2026-02-16 - Mass Assignment via Firestore Update
**Vulnerability:** The `updateUser` endpoint passed the entire JSON request body directly to `db.update()`, allowing users to modify sensitive fields like `isAdmin`, `email`, or internal flags.
**Learning:** Firestore's schema-less nature makes it easy to accidentally allow mass assignment if input validation is skipped. The lack of an explicit data model or serializer exacerbated this.
**Prevention:** Use explicit allow-lists (e.g., `ALLOWED_UPDATE_FIELDS`) for all update operations. Never pass raw user input directly to database write methods.
